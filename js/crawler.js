...
impossibility_detector.isMonsterKillable = function (monster) {
    let certainty = true
    ...
    fakeRoom.monsters = game.player.room.monsters.map((mon, index) => {
        let fakeMon = Object.assign({}, mon)
        fakeMon.id = getGlobalUniqueId()
        return fakeMon
    })

    let fakeMonster = fakeRoom.monsters[game.player.room.monsters.indexOf(monster)]
    ...
    window.arena = fakeRoom
    window.faker = fakePlayer
    ...
    while(fakePlayer.stats.hitpoints > 0 && fakeMonster.hitpoints > 0) {
        ...
        if (fakePlayer.weapon.ammo <= 0) {
            fakePlayer.usedWeaponIds.push(fakePlayer.weapon.id)
            console.log('* FORTA FASCI *')
